...
import { Plus, Loader2, Save, X, MenuSquare, ShieldCheck, ArrowLeft } from "lucide-react";
...
export default function AddPermissionDialog({
  ...
  const validate = () => {
    ...
    if (!formData.group.trim())
      newErrors.group = "กรุณาระบุหรือเลือกหมวดหมู่หลัก";
    setErrors(newErrors);
    ...
    try {
      const res = await fetch(`${apiUrl}/permissions`, {
        ...
        body: JSON.stringify({
          ...
          // 🚀 ถ้าไม่กรอกกลุ่มย่อย ให้จับลง 'ทั่วไป' ป้องกันบัค
          sub_group:
      ...
      const data = await res.json();
      if (res.ok) {
        toast.success("สร้างสิทธิ์การใช้งานเรียบร้อย");
      ...
      <DialogTrigger asChild>
        {isIconOnly ? (
          <button
            ...
            title={`สร้างสิทธิ์ใน ${defaultGroup}`}
          >
            <Plus className="w-5 h-5" />
          </button>
        ) : (
          <Button className="w-full md:w-auto bg-blue-600 hover:bg-blue-900 rounded-full h-10 px-5 gap-2 shadow-lg shadow-blue-600/20 text-sm text-white flex justify-center items-center font-medium transition-all cursor-pointer">
            <Plus className="w-5 h-5 mr-1.5" /> สร้างสิทธิ์ใหม่
          </Button>
        )}
      </DialogTrigger>

      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-hidden rounded-3xl p-0 border-none shadow-2xl bg-white dark:bg-slate-950 flex flex-col [&>button.absolute]:hidden">
        <div className="bg-blue-600 px-8 py-6 text-white flex justify-between items-center z-20 shrink-0">
          <div>
            <DialogTitle className="text-lg font-bold flex items-center gap-3 text-white">
              <ShieldCheck className="w-6 h-6" /> สร้างสิทธิ์การใช้งาน
              (Permission)
            </DialogTitle>
            <p className="text-blue-100 text-[11px] mt-1">
              {defaultGroup
                ? `เพิ่มสิทธิ์ใหม่ล็อกหมวดหมู่ในกลุ่ม "${defaultGroup}"`
                : "กำหนดรหัสสิทธิ์และสร้างหมวดหมู่ใหม่แบบอิสระ"}
            </p>
          </div>
          <button
        ...
        <div className="overflow-y-auto flex-1 p-8 custom-scrollbar">
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <Label className="font-bold text-slate-700 dark:text-slate-300">
                  รหัสสิทธิ์ (ภาษาอังกฤษ){" "}
                  <span className="text-red-500">*</span>
                </Label>
                <Input
                  placeholder="เช่น view_reports, edit_users"
                  value={formData.name}
                  onChange={(e) => {
              ...
              <div className="space-y-3">
                ...
                <GroupComboboxField
                  ...
                  placeholder="เช่น เอกสารขาย, รายงาน, ตั้งค่าระบบ"
                  locked={!!defaultGroup} // ล็อกทันทีถ้ากดจากปุ่ม + บนการ์ด
                  error={!!errors.group}
                  accentColor="blue"
                />
                {errors.group && (
              ...
              <div className="space-y-3">
                ...
                <GroupComboboxField
                  ...
                  placeholder="เช่น ทั่วไป, แท็บ (เว้นว่าง = ทั่วไป)"
                  accentColor="blue"
                />
              </div>
            </div>
            ...
            {formData.is_menu && (
              <div className="p-6 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-900/30 space-y-5 animate-in fade-in slide-in-from-top-4">
                <div className="space-y-3">
                  <Label className="font-bold text-slate-700 dark:text-slate-300">
                    เลือกไอคอน (Icon)
                  </Label>
                  <AppSelect
                    ...
                    triggerClassName="h-12 bg-white dark:bg-slate-900 focus:ring-blue-500"
                ...
                <div className="grid grid-cols-3 gap-6">
                  <div className="col-span-2 space-y-3">
                    ...
                    <Input
                      ...
                      className="h-12 rounded-xl bg-white dark:bg-slate-900 focus:ring-blue-500"
                    />
                  </div>
                  <div className="col-span-1 space-y-3">
                    ...
                    <Input
                      ...
                      className="h-12 rounded-xl bg-white dark:bg-slate-900 text-center focus:ring-blue-500"
                    />
                  </div>
                </div>
              </div>
