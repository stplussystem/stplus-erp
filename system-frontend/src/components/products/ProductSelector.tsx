...
export function ProductSelector({ value, onChange }: ProductSelectorProps) {
  ...
  const fetchProducts = useCallback(async (query: string) => {
    setIsFetching(true);
    try {
      // 💡 เปลี่ยนมาใช้ apiFetch: โค้ดสั้นลง ไม่ต้องเช็ค 401 เอง ไม่ต้องดึง token เอง
      const json = await apiFetch(`/products?search=${query}&per_page=20`);
      if (json && json.data) {
        setProducts(json.data.filter((p: any) => p.product_type === "inventory"));
      }
    } catch (error) {
