                ...
                <tbody className="divide-y divide-slate-100">
                  {rows.map((row, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50">
                      {type === "serial" && (
                        <td className="px-3 py-2.5 font-mono text-slate-700">{row.serial_number}</td>
                      )}
                      <td className="px-3 py-2.5">
                        <div className="font-bold text-slate-800">{row.document_number}</div>
                        <div className="text-[11px] text-slate-400">
                          {DOC_TYPE_LABELS[row.document_type] || row.document_type}
                        </div>
                      </td>
                      <td className="px-3 py-2.5 text-slate-600">{row.party}</td>
                      {type === "aggregate" && (
