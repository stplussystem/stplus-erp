...
export default function MaterialIssueListPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasPermission = perms.some((p: any) =>
        typeof p === "string"
          ? p === "view_material_issue"
          : p?.name === "view_material_issue",
      );
  ...
  const canCreate =
    isSuperAdmin || userPermissions.includes("create_material_issue");
  const canEdit =
    isSuperAdmin || userPermissions.includes("edit_material_issue");
  const canDelete =
    isSuperAdmin || userPermissions.includes("delete_material_issue");
  const canApprove =
    isSuperAdmin || userPermissions.includes("approve_material_issue");
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 print:hidden gap-4">
        ...
        <div className="flex items-center gap-3">
          {canCreate && (
            <Link href="/sales/material-issues/create">
              <button className="flex justify-center h-10 px-5 py-2 w-full md:w-auto gap-2 text-sm font-medium items-center text-white bg-blue-600 hover:bg-blue-700 shadow-sm shadow-blue-600/20 rounded-full cursor-pointer transition-all hover:scale-102 transition-transform disabled:opacity-50">
                <Plus className="w-4 h-4" /> สร้างใบเบิกสินค้า
              </button>
            </Link>
          )}
        </div>
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {canEdit && doc.status === "Pending" && (
                          <AppTooltip label="แก้ไข">
                            <Link
                              href={`/sales/material-issues/${doc.id}/edit`}
                            >
                        ...
                        {canApprove && doc.status === "Pending" && (
                          <AppTooltip label={canEdit ? "แก้ไข / อนุมัติเอกสาร" : "อนุมัติเอกสาร"}>
                            <button
                              ...
                              // เข้า /edit ไม่ได้ (ต้องมี edit_material_issue) จึงยังอนุมัติผ่าน popup ในหน้ารายการเหมือนเดิม
                              onClick={() =>
                                canEdit
                                  ? router.push(`/sales/material-issues/${doc.id}/edit`)
      ...
      <AppConfirmDialog
        ...
        description="ระบบจะตัดสต๊อกออกทันทีเมื่ออนุมัติ ยืนยันการอนุมัติใบเบิกสินค้าฉบับนี้ใช่หรือไม่?"
      ...
      <AppConfirmDialog
        ...
        description="ระบบจะคืนสต๊อกให้ถ้าเคยอนุมัติไปแล้ว ยืนยันการยกเลิกใบเบิกสินค้านี้ใช่หรือไม่?"
