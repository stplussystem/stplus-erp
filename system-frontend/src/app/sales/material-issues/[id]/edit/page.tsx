...
export default function MaterialIssueEditPage() {
  ...
  // ✅ ปุ่มอนุมัติในหน้าแก้ไข — แสดงเฉพาะผู้ที่มีสิทธิ์ approve_material_issue มี popup ยืนยันเหมือนหน้ารายการ
  // ถ้ามีการแก้ไขที่ยังไม่บันทึก ปุ่มอนุมัติจะอัปเดตเอกสารให้ก่อน (บันทึกไม่สำเร็จ = ไม่อนุมัติ)
  const canApprove = usePermission("approve_material_issue");
  ...
  const handleUpdate = async () => {
    if (await persist()) router.push("/sales/material-issues");
  };
