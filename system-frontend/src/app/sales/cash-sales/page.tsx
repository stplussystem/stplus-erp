...
export default function CashSaleListPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasPermission = perms.some((p: any) =>
        typeof p === "string" ? p === "view_cash" : p?.name === "view_cash",
      );
  ...
  const canCreate = isSuperAdmin || userPermissions.includes("create_cash");
  const canEdit = isSuperAdmin || userPermissions.includes("edit_cash");
  const canDelete = isSuperAdmin || userPermissions.includes("delete_cash");
  const canApprove = isSuperAdmin || userPermissions.includes("approve_cash");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/sale-documents?type=cash`, {
        headers: {
  ...
  const executeRevise = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/sales/cash-sales/${result.data.id}/edit`);
      } else {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-[11px] mt-0.5">
              จัดการเอกสารขายเงินสดสำหรับลูกค้า
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {canCreate && (
            <Link href="/sales/cash-sales/create">
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {canEdit && doc.status === "Pending" && (
                          <AppTooltip label="แก้ไข">
                            <Link href={`/sales/cash-sales/${doc.id}/edit`}>
