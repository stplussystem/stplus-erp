...
export default function CashSaleEditPage() {
  ...
  // ปุ่มอนุมัติสีเขียว — แสดงเฉพาะผู้ที่มีสิทธิ์ approve_cash (เอกสารในหน้านี้เป็น Pending เสมอ เพราะ fetchDocumentData เด้งออกถ้าไม่ใช่) มี popup ยืนยัน
  ...
  const canApprove = usePermission("approve_cash");
  ...
  // 🔒 เอกสารนี้ล็อกรายการตามใบเบิกสินค้า (material_issue) หรือไม่ — ตรวจจาก referencedDocument ที่ backend eager-load มาให้
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasPermission = perms.some((p: any) =>
        typeof p === "string" ? p === "edit_cash" : p?.name === "edit_cash",
      );
  ...
  const handleUpdate = async () => {
    if (await persist()) router.push("/sales/cash-sales");
  };
