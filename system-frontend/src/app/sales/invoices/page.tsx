...
export default function InvoiceListPage() {
  ...
  const canCreate = isSuperAdmin || userPermissions.includes("create_invoice");
  const canEdit = isSuperAdmin || userPermissions.includes("edit_invoice");
  const canDelete = isSuperAdmin || userPermissions.includes("delete_invoice");
  const canApprove = isSuperAdmin || userPermissions.includes("approve_invoice");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/sale-documents?type=invoice`, {
        headers: {
  ...
  const executeRevise = async () => {
    ...
    try {
      ...
      const apiUrl =
        process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000/api";
      const res = await fetch(`${apiUrl}/sale-documents/${reviseTarget}/revise`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      if (res.ok) {
        ...
        router.push(`/sales/invoices/${result.data.id}/edit`);
      } else {
  ...
  // 🖨️ [2026-09-16] มิเรอร์ pattern เดียวกับ tax-invoices/receipts list — สร้าง PDF blob โดยบังคับขนาดกระดาษ
  // ที่ระบุเสมอ (ไม่อ้างอิง paperSize หลักของบริษัท) ใช้ร่วมกันทั้งปุ่มพิมพ์ (Letter) และพรีวิว (A4)
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-[11px] mt-0.5">
              จัดการเอกสารใบแจ้งหนี้สำหรับลูกค้า
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {canEdit && doc.status === "Pending" && (
                          <AppTooltip label="แก้ไข">
                            <Link href={`/sales/invoices/${doc.id}/edit`}>
      ...
      <AppConfirmDialog
        ...
        icon={CheckCircle2}
        iconColorClass="bg-blue-50 text-blue-600 border-blue-100/50"
        title="อนุมัติใบแจ้งหนี้?"
        description="ยืนยันการอนุมัติใบแจ้งหนี้ฉบับนี้เพื่อส่งให้ลูกค้าใช่หรือไม่?"
        ...
        onSecondary={() => {
          ...
          setApproveTarget(null);
          if (id) router.push(`/sales/invoices/${id}/edit`);
        }}
      />
      ...
      <AppConfirmDialog
        ...
        onOpenChange={(v) => {
          if (!v) { setCancelTarget(null); setCancelReason(""); }
        }}
        icon={XCircle}
        iconColorClass="bg-orange-50 text-orange-600 border-orange-100/50"
        title="ยกเลิกใบแจ้งหนี้?"
        description="ยืนยันการยกเลิกใบแจ้งหนี้นี้ใช่หรือไม่?"
      ...
      >
        <label className="block text-xs font-bold text-muted-foreground mb-2 uppercase tracking-wider">เหตุผลในการยกเลิก</label>
        <input
          type="text"
