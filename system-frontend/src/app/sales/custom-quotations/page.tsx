...
export default function CustomQuotationListPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasPermission = perms.some((p: any) =>
        typeof p === "string"
          ? p === "view_custom_quotation"
          : p?.name === "view_custom_quotation",
      );
  ...
  const canCreate =
    isSuperAdmin || userPermissions.includes("create_custom_quotation");
  const canEdit =
    isSuperAdmin || userPermissions.includes("edit_custom_quotation");
  const canDelete =
    isSuperAdmin || userPermissions.includes("delete_custom_quotation");
  const canApprove =
    isSuperAdmin || userPermissions.includes("approve_custom_quotation");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const apiUrl =
        process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000/api";
      // 🚀 ดึงเฉพาะใบเสนอราคาแบบกำหนดเองเท่านั้น (แยกจาก quotation หลักโดยสิ้นเชิง)
      const res = await fetch(
        `${apiUrl}/sale-documents?type=custom_quotation`,
        {
          headers: {
  ...
  const executeRevise = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/sales/custom-quotations/${result.data.id}/edit`);
      } else {
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-[11px] mt-0.5">
              ใบเสนอราคาสำหรับงานเช่าที่ปรับโลโก้/ชื่อบริษัท/ผู้เสนอราคาได้เฉพาะเอกสาร
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {canCreate && (
            <Link href="/sales/custom-quotations/create">
              <Button className="flex justify-center h-10 px-5 py-2 w-full md:w-auto gap-2 text-sm font-medium items-center text-white bg-fuchsia-600 hover:bg-fuchsia-700 shadow-sm shadow-fuchsia-600/20 rounded-full cursor-pointer transition-all hover:scale-102 transition-transform disabled:opacity-50">
        ...
        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div className="relative w-full max-w-sm">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 rounded-xl h-10 border border-slate-200 focus:border-fuchsia-500 focus:ring-2 focus:ring-fuchsia-100 outline-none text-sm transition-colors"
            />
          </div>
        </div>
              ...
              {loading ? (
                ...
                paginatedDocs.map((doc) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right font-bold text-fuchsia-600">
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {canEdit && doc.status === "Pending" && (
                          <AppTooltip label="แก้ไข">
                            <Link
                              href={`/sales/custom-quotations/${doc.id}/edit`}
                            >
      ...
      <AppConfirmDialog
        ...
        description="ยืนยันการอนุมัติใบเสนอราคาฉบับนี้เพื่อส่งให้ลูกค้าใช่หรือไม่?"
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl h-[90vh] shadow-2xl flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800 flex items-center gap-2">
                <FileText className="w-5 h-5 text-fuchsia-500" /> เอกสาร PDF
              </h3>
              <button
                onClick={() => {
