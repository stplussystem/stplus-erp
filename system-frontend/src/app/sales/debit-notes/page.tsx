...
export default function DebitNoteListPage() {
  ...
  const canCreate =
    isSuperAdmin || userPermissions.includes("create_debit_note");
  const canEdit = isSuperAdmin || userPermissions.includes("edit_debit_note");
  const canDelete =
    isSuperAdmin || userPermissions.includes("delete_debit_note");
  const canApprove =
    isSuperAdmin || userPermissions.includes("approve_debit_note");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/sale-documents?type=debit_note`, {
        headers: {
  ...
  const executeRevise = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/sales/debit-notes/${result.data.id}/edit`);
      } else {
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-[11px] mt-0.5">
              จัดการเอกสารใบเพิ่มหนี้สำหรับลูกค้า
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {canCreate && (
            <Link href="/sales/debit-notes/create">
              <button className="flex justify-center h-10 px-5 py-2 w-full md:w-auto gap-2 text-sm font-medium items-center text-white bg-blue-600 hover:bg-blue-700 shadow-sm shadow-blue-600/20 rounded-full cursor-pointer transition-all hover:scale-102 transition-transform disabled:opacity-50">
                <Plus className="w-4 h-4" /> สร้างใบเพิ่มหนี้
              </button>
            </Link>
          )}
        </div>
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {canEdit && doc.status === "Pending" && (
                          <AppTooltip label="แก้ไข">
                            <Link href={`/sales/debit-notes/${doc.id}/edit`}>
      ...
      <AppConfirmDialog
        ...
        icon={CheckCircle2}
        iconColorClass="bg-blue-50 text-blue-600 border-blue-100/50"
        title="อนุมัติใบเพิ่มหนี้?"
        description="ยืนยันการอนุมัติใบเพิ่มหนี้ฉบับนี้ใช่หรือไม่?"
      ...
      <AppConfirmDialog
        ...
        title="ยกเลิกใบเพิ่มหนี้?"
        description="ยืนยันการยกเลิกใบเพิ่มหนี้นี้ใช่หรือไม่?"
