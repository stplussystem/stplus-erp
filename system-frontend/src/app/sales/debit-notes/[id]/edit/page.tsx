...
export default function DebitNoteEditPage() {
  ...
  // ปุ่มอนุมัติสีเขียว — แสดงเฉพาะผู้ที่มีสิทธิ์ approve_debit_note (หน้านี้โหลดได้เฉพาะเอกสาร Pending — ถ้าไม่ใช่จะ redirect ออก)
  // ถ้ามีการแก้ไขที่ยังไม่บันทึก ปุ่มอนุมัติจะอัปเดตเอกสารให้ก่อน (บันทึกไม่สำเร็จ = ไม่อนุมัติ)
  const canApprove = usePermission("approve_debit_note");
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (isPlatformAdmin || isSuper || hasPermission) {
        ...
        router.replace("/sales/debit-notes");
      }
    } catch (e) {
      router.replace("/");
    }
  ...
  const handleUpdate = async () => {
    if (await persist()) router.push("/sales/debit-notes");
  };
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 overflow-x-hidden text-foreground pb-20">
      ...
      <div className="bg-card p-6 rounded-2xl shadow-sm border border-border min-h-[500px]">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-8 p-5 border border-border rounded-xl bg-muted/50">
          <div className="md:col-span-2">
            ...
            <input
              type="text"
              className="w-full h-10 text-sm px-4 rounded-xl border border-blue-200 bg-muted text-muted-foreground font-bold outline-none cursor-not-allowed"
              value="ใบเพิ่มหนี้ (DN)"
              disabled
            />
          </div>
          <div>
      ...
      <AppConfirmDialog
        ...
        description={
          <>
            ยืนยันการอนุมัติใบเพิ่มหนี้ฉบับนี้ใช่หรือไม่?
            {hasUnsavedChanges && (
