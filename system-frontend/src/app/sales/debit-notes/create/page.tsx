...
export default function DebitNoteCreatePage() {
  ...
  // 🚀 เลือกใบกำกับภาษีที่อนุมัติแล้ว มาโหลดลูกค้า/รายละเอียด/รายการสินค้าให้อัตโนมัติ (ใบเพิ่มหนี้อ้างอิงใบกำกับภาษีต้นทาง)
  const handleSelectTaxInvoice = async (taxInvoiceId: string) => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/sale-documents/${taxInvoiceId}`, {
  ...
  const handlePreviewPDF = async () => {
    ...
    try {
      ...
      const { paperSize, letterLayout } = getPaperSizeConfig(companySettings, "debit_note");
    ...
    <div className="w-full max-w-full px-4 py-4 text-foreground">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 print:hidden gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <Link href="/sales/debit-notes" className="w-full md:w-auto">
            <button
              type="button"
              className="flex justify-center h-10 px-5 py-2 w-full md:w-auto gap-2 text-sm font-medium items-center text-slate-700 bg-white hover:bg-slate-100 border border-slate-200 hover:border-slate-300 shadow-sm rounded-full cursor-pointer transition-all hover:scale-102 transition-transform"
            >
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 min-h-[500px]">
        ...
        <div className="grid grid-cols-1 gap-5 mb-8 p-5 border border-slate-100 rounded-xl bg-slate-50/50">
          <div>
