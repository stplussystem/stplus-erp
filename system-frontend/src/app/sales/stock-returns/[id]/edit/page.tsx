...
// 🧾 คืนสินค้าที่ขายไปแล้ว อ้างอิงใบลดหนี้เท่านั้น (แยกจาก sales/rental-stock-returns แล้ว)
...
export default function StockReturnEditPage() {
  ...
  // ปุ่มอนุมัติสีเขียว — แสดงเฉพาะผู้ที่มีสิทธิ์ approve_stock_return (เอกสารในหน้านี้เป็น Pending เสมอ เพราะ fetch จะ redirect ออกถ้าไม่ใช่)
  ...
  const canApprove = usePermission("approve_stock_return");
  ...
  const handleUpdate = async () => {
    if (await persist()) router.push("/sales/stock-returns");
  };
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
      ...
      <AppConfirmDialog
        ...
        description={
          <>
            ระบบจะรับสต๊อกคืนทันทีเมื่ออนุมัติ ยืนยันการอนุมัติใบคืนสินค้าฉบับนี้ใช่หรือไม่?
            {hasUnsavedChanges && (
