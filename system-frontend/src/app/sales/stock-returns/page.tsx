...
export default function StockReturnListPage() {
  ...
  const canCreate =
    isSuperAdmin || userPermissions.includes("create_stock_return");
  const canEdit = isSuperAdmin || userPermissions.includes("edit_stock_return");
  const canDelete =
    isSuperAdmin || userPermissions.includes("delete_stock_return");
  const canApprove =
    isSuperAdmin || userPermissions.includes("approve_stock_return");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/sale-documents?type=stock_return`, {
        headers: {
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-md font-bold tracking-tight">
              ใบคืนสินค้า (จากใบลดหนี้)
            </h1>
            <p className="text-muted-foreground text-[11px] mt-0.5">
              รับคืนสินค้าที่ขายไปแล้ว อ้างอิงใบลดหนี้
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {canCreate && (
            <Link href="/sales/stock-returns/create">
          ...
          <table className="w-full text-sm text-left whitespace-nowrap">
            <thead className="text-xs text-muted-foreground uppercase bg-muted/50 border-b border-border">
              <tr>
                ...
                <th className="px-6 py-4 font-bold">อ้างอิงใบลดหนี้</th>
                      ...
                      <div className="flex items-center justify-center gap-1">
                        {canEdit && doc.status === "Pending" && (
                          <AppTooltip label="แก้ไข">
                            <Link href={`/sales/stock-returns/${doc.id}/edit`}>
      ...
      <AppConfirmDialog
        ...
        title="อนุมัติใบคืนสินค้า?"
        description="ระบบจะรับสต๊อกคืนทันทีเมื่ออนุมัติ ยืนยันการอนุมัติใบคืนสินค้าฉบับนี้ใช่หรือไม่?"
        ...
        onSecondary={() => {
          ...
          setApproveTarget(null);
          if (id) router.push(`/sales/stock-returns/${id}/edit`);
        }}
      />
      ...
      <AppConfirmDialog
        ...
        title="ยกเลิกใบคืนสินค้า?"
        description="ยืนยันการยกเลิกใบคืนสินค้านี้ใช่หรือไม่?"
