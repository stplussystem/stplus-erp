...
// 🏠 คืนอุปกรณ์เช่า อ้างอิงใบเบิกสินค้า (stock_issue) เท่านั้น — แยกออกจาก sales/stock-returns เดิม
// (ที่เหลือเฉพาะคืนจากใบลดหนี้ ฝั่ง "ขาย") ให้เป็น document_type ของตัวเอง (rental_stock_return) เพื่อให้
// scope สิทธิ์ role แบบ "พนักงานคืนสินค้าเช่า" ได้จริง โดยไม่พ่วงสิทธิ์คืนสินค้าจากใบลดหนี้มาด้วย
export default function RentalStockReturnListPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasPermission = perms.some((p: any) =>
        typeof p === "string"
          ? p === "view_rental_stock_return"
          : p?.name === "view_rental_stock_return",
      );
  ...
  const canEdit =
    isSuperAdmin || userPermissions.includes("edit_rental_stock_return");
  const canDelete =
    isSuperAdmin || userPermissions.includes("delete_rental_stock_return");
  const canApprove =
    isSuperAdmin || userPermissions.includes("approve_rental_stock_return");
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-md font-bold tracking-tight">
              ใบคืนสินค้าเช่า (Rental Stock Return)
            </h1>
            <p className="text-muted-foreground text-[11px] mt-0.5">
              รับคืนอุปกรณ์เช่าอ้างอิงใบเบิกสินค้าของแต่ละงานเช่า
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {canCreate && (
            <Link href="/sales/rental-stock-returns/create">
              <button className="flex justify-center h-10 px-5 py-2 w-full md:w-auto gap-2 text-sm font-medium items-center text-white bg-blue-600 hover:bg-blue-700 shadow-sm shadow-blue-600/20 rounded-full cursor-pointer transition-all hover:scale-102 transition-transform disabled:opacity-50">
                <Plus className="w-4 h-4" /> สร้างใบคืนสินค้าเช่า
              </button>
            </Link>
          )}
        </div>
          ...
          <table className="w-full text-sm text-left whitespace-nowrap">
            <thead className="text-xs text-muted-foreground uppercase bg-muted/50 border-b border-border">
              <tr>
                ...
                <th className="px-6 py-4 font-bold">อ้างอิงใบเบิก</th>
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {canEdit && doc.status === "Pending" && (
                          <AppTooltip label="แก้ไข">
                            <Link
                              href={`/sales/rental-stock-returns/${doc.id}/edit`}
                            >
      ...
      <AppConfirmDialog
        ...
        description="ระบบจะปลดจองสต๊อกทันทีเมื่ออนุมัติ ยืนยันการอนุมัติใบคืนสินค้าเช่าฉบับนี้ใช่หรือไม่?"
        ...
        onSecondary={() => {
          ...
          setApproveTarget(null);
          if (id) router.push(`/sales/rental-stock-returns/${id}/edit`);
        }}
      />
      ...
      <AppConfirmDialog
        ...
        title="ยกเลิกใบคืนสินค้าเช่า?"
        description="ยืนยันการยกเลิกใบคืนสินค้าเช่านี้ใช่หรือไม่?"
