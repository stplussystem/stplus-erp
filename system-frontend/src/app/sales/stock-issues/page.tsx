...
export default function StockIssueListPage() {
  ...
  const canCreate =
    isSuperAdmin || userPermissions.includes("create_stock_issue");
  const canEdit = isSuperAdmin || userPermissions.includes("edit_stock_issue");
  const canDelete =
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/sale-documents?type=stock_issue`, {
        headers: {
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 print:hidden gap-4">
        ...
        <div className="flex items-center gap-3">
          {canCreate && (
            <Link href="/sales/stock-issues/create">
              <button className="flex justify-center h-10 px-5 py-2 w-full md:w-auto gap-2 text-sm font-medium items-center text-white bg-blue-600 hover:bg-blue-700 shadow-sm shadow-blue-600/20 rounded-full cursor-pointer transition-all hover:scale-102 transition-transform disabled:opacity-50">
                <Plus className="w-4 h-4" /> สร้างใบเบิกสินค้าเช่า
              </button>
            </Link>
          )}
        </div>
          ...
          <table className="w-full text-sm text-left whitespace-nowrap">
            <thead className="text-xs text-muted-foreground uppercase bg-muted/50 border-b border-border">
              <tr>
                ...
                <th className="px-6 py-4 font-bold">งานเช่า</th>
      ...
      <AppConfirmDialog
        ...
        onSecondary={() => {
          ...
          setApproveTarget(null);
          if (id) router.push(`/sales/stock-issues/${id}/edit`);
        }}
      />
      ...
      <AppConfirmDialog
        ...
        title="ยกเลิกใบเบิกสินค้าเช่า?"
        description="ยืนยันการยกเลิกใบเบิกสินค้าเช่านี้ใช่หรือไม่?"
