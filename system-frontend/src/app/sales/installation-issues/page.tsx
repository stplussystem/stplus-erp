...
export default function InstallationIssueListPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  // 🆕 [2026-09-18] มาจากรายงานกำไร-ขาดทุนต่อโครงการ (คลิก "ค่าสินค้าติดตั้ง") — กรองเหลือเฉพาะใบเบิกของโครงการนั้น
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasPermission = perms.some((p: any) =>
        typeof p === "string"
          ? p === "view_installation_issue"
          : p?.name === "view_installation_issue",
      );
  ...
  const canCreate =
    isSuperAdmin || userPermissions.includes("create_installation_issue");
  const canEdit =
    isSuperAdmin || userPermissions.includes("edit_installation_issue");
  const canDelete =
    isSuperAdmin || userPermissions.includes("delete_installation_issue");
  const canApprove =
    isSuperAdmin || userPermissions.includes("approve_installation_issue");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${apiUrl}/sale-documents?type=installation_issue${projectParam}`,
        {
          headers: {
  ...
  const executeRevise = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/sales/installation-issues/${result.data.id}/edit`);
      } else {
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-md font-bold tracking-tight">
              ใบเบิกวัสดุติดตั้ง (Installation Issue)
            </h1>
            <p className="text-muted-foreground text-[11px] mt-0.5">
              เบิกวัสดุ/บริการสำหรับติดตั้งงานโครงการ — ตัดสต๊อกจริงเมื่ออนุมัติ
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {canCreate && (
            <Link href="/sales/installation-issues/create">
              <button className="flex justify-center h-10 px-5 py-2 w-full md:w-auto gap-2 text-sm font-medium items-center text-white bg-blue-600 hover:bg-blue-700 shadow-sm shadow-blue-600/20 rounded-full cursor-pointer transition-all hover:scale-102 transition-transform disabled:opacity-50">
                <Plus className="w-4 h-4" /> สร้างใบเบิกวัสดุติดตั้ง
              </button>
            </Link>
          )}
        </div>
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {canEdit && doc.status === "Pending" && (
                          <AppTooltip label="แก้ไข">
                            <Link
                              href={`/sales/installation-issues/${doc.id}/edit`}
                            >
      ...
      <AppConfirmDialog
        ...
        description="ระบบจะโคลนข้อมูลเป็นใบใหม่และเข้าสู่หน้าแก้ไขทันที ถ้าใบเดิมอนุมัติแล้วจะคืนสต๊อกให้ และต้องอนุมัติใบใหม่อีกครั้ง"
      ...
      <AppConfirmDialog
        ...
        description="ระบบจะตัดสต๊อกออกทันทีเมื่ออนุมัติ ยืนยันการอนุมัติใบเบิกวัสดุติดตั้งฉบับนี้ใช่หรือไม่?"
        ...
        onSecondary={() => {
          ...
          setApproveTarget(null);
          if (id) router.push(`/sales/installation-issues/${id}/edit`);
        }}
      />
      ...
      <AppConfirmDialog
        ...
        title="ยกเลิกใบเบิกวัสดุติดตั้ง?"
        description="ระบบจะคืนสต๊อกให้ถ้าเคยอนุมัติไปแล้ว ยืนยันการยกเลิกใบเบิกวัสดุติดตั้งนี้ใช่หรือไม่?"
      ...
      >
        ...
        <input
          type="text"
          placeholder="เช่น เปลี่ยนแผนใช้งาน"
