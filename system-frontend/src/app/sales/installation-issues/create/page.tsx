...
export default function InstallationIssueCreatePage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasPermission = perms.some((p: any) =>
        typeof p === "string"
          ? p === "create_installation_issue"
          : p?.name === "create_installation_issue",
      );
  ...
  // 🚀 เลือกโครงการแล้ว auto-fill ลูกค้าจาก project.contact_id (ถ้ามี)
  ...
  // 🚀 เติมโครงการอัตโนมัติเมื่อมาจากปุ่ม "บันทึกวัสดุที่เบิกไปติดตั้ง" ในหน้าบันทึกการติดตั้ง (?project_id=)
  useEffect(() => {
  ...
  // 📌 ไม่มีส่วนลด/ภาษีสำหรับใบเบิกวัสดุติดตั้ง (tax_type ล็อกเป็น "none" เสมอ)
  ...
  const documentNumberPreview = useMemo(() => {
    ...
    if (companySettings?.document_settings) {
      ...
      prefix = settings?.docs?.installation_issue?.prefix || "IM";
      prefixSep =
