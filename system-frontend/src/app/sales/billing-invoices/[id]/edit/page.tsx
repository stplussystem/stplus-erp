...
export default function BillingInvoiceEditPage() {
  ...
  // ปุ่มอนุมัติสีเขียว — แสดงเฉพาะผู้ที่มีสิทธิ์ approve_billing_invoice (เอกสารในหน้านี้เป็น Pending เสมอ เพราะ fetchDocumentData เด้งออกถ้าไม่ใช่) มี popup ยืนยัน
  ...
  const canApprove = usePermission("approve_billing_invoice");
  ...
  // 🔒 เอกสารเก่า (ก่อนโหมดอ้างอิงใบกำกับภาษีหลายใบ) ยังคงแก้ไขแบบตารางสินค้าเดิมได้ตามปกติ ไม่บังคับ migrate ข้อมูล
  const [isLegacyItemsDoc, setIsLegacyItemsDoc] = useState(false);
  const {
    items,
    loadFromDocument,
    selectProduct,
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasPermission = perms.some((p: any) =>
        typeof p === "string"
          ? p === "edit_billing_invoice"
          : p?.name === "edit_billing_invoice",
      );
      ...
      if (isPlatformAdmin || isSuper || hasPermission) {
        ...
        router.replace("/sales/billing-invoices");
      }
    } catch (e) {
      router.replace("/");
    }
  ...
  const fetchDocumentData = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const invoiceRefs = doc.invoice_refs || [];
        if (invoiceRefs.length > 0) {
          setRefRows(
            invoiceRefs.map((r: any) => ({
  ...
  const finance = isLegacyItemsDoc ? legacyFinance : newFinance;
      ...
      const blob = await pdf(
        <SalesPdfTemplate
          data={{
            ...
            invoiceRefs: isLegacyItemsDoc ? [] : refRows,
            finance,
  ...
  // snapshot เฉพาะค่าที่ผู้ใช้แก้จริง (ใบกำกับที่อ้างอิงเก็บแค่ tax_invoice_id ตาม payload — ไม่รวม outstanding_balance ที่เปลี่ยนทีหลังโหลด)
  const currentSnapshot = useMemo(
    () =>
      JSON.stringify({
        formData,
        refs: refRows.map((r) => r.tax_invoice_id),
        items: isLegacyItemsDoc ? items : [],
      }),
  ...
  const persist = async (): Promise<boolean> => {
    ...
    try {
      ...
      if (isLegacyItemsDoc) {
        payload.vat_amount = (finance as typeof legacyFinance).vat_amount;
        payload.wht_amount = (finance as typeof legacyFinance).wht_amount;
        payload.items = buildPayload();
      } else {
  ...
  const handleUpdate = async () => {
    if (await persist()) router.push("/sales/billing-invoices");
  };
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 overflow-x-hidden text-foreground mx-auto space-y-6 antialiased">
            ...
            {isLegacyItemsDoc && (
              <>
                ...
                <div className="flex justify-between font-medium pt-2">
                  ...
                  <span>
                    {legacyFinance.subtotal.toLocaleString(undefined, {
                ...
                {legacyFinance.discount > 0 && (
                  <div className="flex justify-between text-foreground font-medium">
                    <span>ยอดหลังหักส่วนลด</span>
                    <span>
                      {legacyFinance.after_discount.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                      })}
                    </span>
                  </div>
                ...
                {formData.tax_type !== "none" && (
                  <div className="flex justify-between font-medium">
                    ...
                    <span>
                      {legacyFinance.vat_amount.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                      })}
                    </span>
                  </div>
      ...
      <AppConfirmDialog
        ...
        title="อนุมัติใบวางบิล?"
        description={
          <>
            ยืนยันการอนุมัติใบวางบิลฉบับนี้เพื่อส่งให้ลูกค้าใช่หรือไม่?
            {hasUnsavedChanges && (
