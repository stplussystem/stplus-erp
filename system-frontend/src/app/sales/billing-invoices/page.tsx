...
export default function BillingInvoiceListPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasPermission = perms.some((p: any) =>
        typeof p === "string"
          ? p === "view_billing_invoice"
          : p?.name === "view_billing_invoice",
      );
  ...
  const canCreate =
    isSuperAdmin || userPermissions.includes("create_billing_invoice");
  const canEdit =
    isSuperAdmin || userPermissions.includes("edit_billing_invoice");
  const canDelete =
    isSuperAdmin || userPermissions.includes("delete_billing_invoice");
  const canApprove =
    isSuperAdmin || userPermissions.includes("approve_billing_invoice");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/sale-documents?type=billing_invoice`, {
        headers: {
  ...
  const executeRevise = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/sales/billing-invoices/${result.data.id}/edit`);
      } else {
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-md font-bold tracking-tight">
              ใบวางบิล (Billing Invoice)
            </h1>
            <p className="text-slate-500 text-[11px] mt-0.5">
              จัดการเอกสารใบวางบิลสำหรับลูกค้า
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {canCreate && (
            <Link href="/sales/billing-invoices/create">
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {canEdit && doc.status === "Pending" && (
                          <AppTooltip label="แก้ไข">
                            <Link
                              href={`/sales/billing-invoices/${doc.id}/edit`}
                            >
      ...
      <AppConfirmDialog
        ...
        description="ยืนยันการอนุมัติใบวางบิลฉบับนี้เพื่อส่งให้ลูกค้าใช่หรือไม่?"
      ...
      <AppConfirmDialog
        ...
        description="ยืนยันการยกเลิกใบวางบิลนี้ใช่หรือไม่?"
