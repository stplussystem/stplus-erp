...
export default function ReceiptListPage() {
  ...
  // 💰 ยอดค้างชำระต่อใบกำกับภาษี — ใช้เติมคอลัมน์ "ยอดคงค้าง" ตอนพิมพ์ใบเสร็จแบบอ้างอิงหลายใบจากหน้า list นี้
  ...
  const canCreate = isSuperAdmin || userPermissions.includes("create_receipt");
  const canEdit = isSuperAdmin || userPermissions.includes("edit_receipt");
  const canDelete = isSuperAdmin || userPermissions.includes("delete_receipt");
  const canApprove =
    isSuperAdmin || userPermissions.includes("approve_receipt");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/sale-documents?type=receipt`, {
        headers: {
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-[11px] mt-0.5">
              จัดการเอกสารใบเสร็จรับเงินสำหรับลูกค้า
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {canCreate && (
            <Link href="/sales/receipts/create">
              <button className="flex justify-center h-10 px-5 py-2 w-full md:w-auto gap-2 text-sm font-medium items-center text-white bg-blue-600 hover:bg-blue-700 shadow-sm shadow-blue-600/20 rounded-full cursor-pointer transition-all hover:scale-102 transition-transform disabled:opacity-50">
                <Plus className="w-4 h-4" /> สร้างใบเสร็จรับเงิน
              </button>
            </Link>
          )}
        </div>
      ...
      <AppConfirmDialog
        ...
        description="ระบบจะตัดสต๊อกออกทันทีเมื่ออนุมัติ ยืนยันการอนุมัติใบเสร็จรับเงินฉบับนี้ใช่หรือไม่?"
      ...
      <AppConfirmDialog
        ...
        description="ระบบจะคืนสต๊อกให้ถ้าเคยอนุมัติไปแล้ว ยืนยันการยกเลิกใบเสร็จรับเงินนี้ใช่หรือไม่?"
