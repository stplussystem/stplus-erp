...
export default function ReceiptEditPage() {
  ...
  // 🔁 เติมยอดค้างชำระปัจจุบันให้แถวที่โหลดมาจากเอกสารเดิม (หลัง balanceById โหลดเสร็จ)
  useEffect(() => {
    if (Object.keys(balanceById).length === 0 || refRows.length === 0) return;
    setRefRows((prev) =>
      prev.map((r) => ({
        ...r,
        outstanding_balance:
          balanceById[r.tax_invoice_id] ?? r.outstanding_balance,
      })),
    );
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
      ...
      {isLegacyItemsDoc && serialPickerIndex !== null && (
        <SerialPickerDialog
