...
export default function TaxInvoiceListPage() {
  ...
  const canCreate =
    isSuperAdmin || userPermissions.includes("create_tax_invoice");
  const canEdit = isSuperAdmin || userPermissions.includes("edit_tax_invoice");
  const canDelete =
    isSuperAdmin || userPermissions.includes("delete_tax_invoice");
  const canApprove =
    isSuperAdmin || userPermissions.includes("approve_tax_invoice");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/sale-documents?type=tax_invoice`, {
        headers: {
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-[11px] mt-0.5">
              จัดการเอกสารใบกำกับภาษีสำหรับลูกค้า
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {canCreate && (
            <Link href="/sales/tax-invoices/create">
      ...
      <AppConfirmDialog
        ...
        description="ระบบจะตัดสต๊อกออกทันทีเมื่ออนุมัติ ยืนยันการอนุมัติใบกำกับภาษีฉบับนี้เพื่อส่งให้ลูกค้าใช่หรือไม่?"
      ...
      <AppConfirmDialog
        ...
        description="ระบบจะคืนสต๊อกให้ถ้าเคยอนุมัติไปแล้ว ยืนยันการยกเลิกใบกำกับภาษีนี้ใช่หรือไม่?"
