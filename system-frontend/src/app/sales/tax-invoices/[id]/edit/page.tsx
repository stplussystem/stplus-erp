...
export default function TaxInvoiceEditPage() {
  ...
  // ✅ ปุ่มอนุมัติสีเขียวในหน้าแก้ไข — แสดงเฉพาะผู้ที่มีสิทธิ์ approve_tax_invoice และเอกสารยังรออนุมัติ มี popup ยืนยันเหมือนหน้ารายการ
  // ถ้ามีการแก้ไขที่ยังไม่บันทึก ปุ่มอนุมัติจะอัปเดตเอกสารให้ก่อน (บันทึกไม่สำเร็จ = ไม่อนุมัติ ไม่ตัดสต๊อก)
  const canApprove = usePermission("approve_tax_invoice");
  ...
  const handleUpdate = async () => {
    if (await persist()) router.push("/sales/tax-invoices");
  };
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
      ...
      <AppConfirmDialog
        ...
        description={
          <>
            ระบบจะตัดสต๊อกออกทันทีเมื่ออนุมัติ ยืนยันการอนุมัติใบกำกับภาษีฉบับนี้เพื่อส่งให้ลูกค้าใช่หรือไม่?
            {hasUnsavedChanges && (
