...
export default function DeliveryNoteListPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasPermission = perms.some((p: any) =>
        typeof p === "string"
          ? p === "view_delivery_note"
          : p?.name === "view_delivery_note",
      );
  ...
  const canCreate =
    isSuperAdmin || userPermissions.includes("create_delivery_note");
  const canEdit =
    isSuperAdmin || userPermissions.includes("edit_delivery_note");
  const canDelete =
    isSuperAdmin || userPermissions.includes("delete_delivery_note");
  const canApprove =
    isSuperAdmin || userPermissions.includes("approve_delivery_note");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const apiUrl =
        process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000/api";
      const res = await fetch(`${apiUrl}/sale-documents?type=delivery_note`, {
        headers: {
  ...
  const executeRevise = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/sales/delivery-notes/${result.data.id}/edit`);
      } else {
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {canEdit && doc.status === "Pending" && (
                          <AppTooltip label="แก้ไข">
                            <Link href={`/sales/delivery-notes/${doc.id}/edit`}>
