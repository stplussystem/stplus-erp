...
export default function DeliveryNoteEditPage() {
  ...
  // ปุ่มอนุมัติสีเขียว — แสดงเฉพาะผู้ที่มีสิทธิ์ approve_delivery_note (หน้านี้โหลดได้เฉพาะเอกสาร Pending — ถ้าไม่ใช่จะ redirect ออก)
  // ถ้ามีการแก้ไขที่ยังไม่บันทึก ปุ่มอนุมัติจะอัปเดตเอกสารให้ก่อน (บันทึกไม่สำเร็จ = ไม่อนุมัติ)
  const canApprove = usePermission("approve_delivery_note");
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasPermission = perms.some((p: any) =>
        typeof p === "string"
          ? p === "edit_delivery_note"
          : p?.name === "edit_delivery_note",
      );
  ...
  // snapshot เฉพาะค่าที่ผู้ใช้แก้จริง (formData + รายการสินค้า) — isLockedToMaterialIssue ไม่รวม เพราะเป็นค่าที่ตั้งตอนโหลดเอกสาร ผู้ใช้แก้เองไม่ได้
  ...
  const handleUpdate = async () => {
    if (await persist()) router.push("/sales/delivery-notes");
  };
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 overflow-x-hidden text-foreground pb-20">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-xs mt-0.5">
              แก้ไขรายละเอียดใบส่งสินค้าชั่วคราว
            </p>
          </div>
        </div>
      ...
      <AppConfirmDialog
        ...
        description={
          <>
            ยืนยันการอนุมัติใบส่งสินค้าชั่วคราวฉบับนี้ใช่หรือไม่?
            {hasUnsavedChanges && (
