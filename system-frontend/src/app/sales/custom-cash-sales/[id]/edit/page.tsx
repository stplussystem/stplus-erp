...
export default function CustomCashSaleEditPage() {
  ...
  // ปุ่มอนุมัติสีเขียว — แสดงเฉพาะผู้ที่มีสิทธิ์ approve_custom_cash (เอกสารในหน้านี้เป็น Pending เสมอ เพราะ fetchDocumentData เด้งออกถ้าไม่ใช่) มี popup ยืนยัน
  // ถ้ามีการแก้ไขที่ยังไม่บันทึก ปุ่มอนุมัติจะอัปเดตเอกสารให้ก่อน (บันทึกไม่สำเร็จ = ไม่อนุมัติ) — pattern เดียวกับ receipts/[id]/edit
  const canApprove = usePermission("approve_custom_cash");
  ...
  const { items, loadFromDocument, selectProduct, updateItem, updateItemSerials, addItem, removeItem, buildPayload } =
  ...
  // snapshot เฉพาะค่าที่ผู้ใช้แก้จริง (formData, รายการสินค้า, Subtotal/VAT ที่พิมพ์ทับ, โลโก้เฉพาะเอกสาร)
  const currentSnapshot = useMemo(
    ...
    [formData, items, manualSubtotal, manualVatAmount, customLogoPath],
  );
  ...
  const handleUpdate = async () => {
    if (await persist()) router.push("/sales/custom-cash-sales");
  };
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-md font-bold tracking-tight flex items-center gap-2">
              แก้ไข{" "}
              <span className="text-teal-600">{formData.document_number}</span>
            </h1>
            <p className="text-muted-foreground text-[11px] mt-0.5">
              แก้ไขรายละเอียดบิลเงินสด (กำหนดเอง)
            </p>
          </div>
        </div>
      ...
      <div className="bg-card p-6 rounded-2xl shadow-sm border border-border min-h-[500px]">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8 p-5 border border-teal-100 rounded-xl bg-teal-50/40">
          <div>
            ...
            {customLogoUrl ? (
              ...
              <button
                ...
                className="w-full h-10 px-4 rounded-xl border border-dashed border-teal-300 bg-background text-xs font-bold text-teal-600 hover:bg-teal-50 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
              >
                {uploadingLogo ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
          ...
          <div>
            ...
            <input
              type="text"
              className="w-full h-10 px-4 text-sm rounded-xl border border-border focus:border-teal-500 focus:ring-2 focus:ring-teal-100 outline-none"
          ...
          <div className="md:col-span-3">
            ...
            <textarea
              rows={2}
              className="w-full p-3 text-sm rounded-xl border border-border outline-none resize-none focus:border-teal-500 focus:ring-2 focus:ring-teal-100"
            ...
            <div className="flex items-center gap-2">
              ...
              <AppTooltip label="เพิ่มลูกค้าใหม่แบบด่วน">
                <button
                  ...
                  className="h-10 w-10 flex items-center justify-center rounded-xl border border-border bg-background text-muted-foreground hover:text-teal-600 hover:border-teal-300 hover:bg-teal-50 cursor-pointer transition-all shrink-0"
                >
      ...
      <AppConfirmDialog
        ...
        title="อนุมัติบิลเงินสด?"
        description={
          <>
            ระบบจะตัดสต๊อกออกทันทีเมื่ออนุมัติ ยืนยันการอนุมัติบิลเงินสดฉบับนี้ใช่หรือไม่?
            {hasUnsavedChanges && (
