...
export default function CustomCashSaleListPage() {
  ...
  const canCreate =
    isSuperAdmin || userPermissions.includes("create_custom_cash");
  const canEdit = isSuperAdmin || userPermissions.includes("edit_custom_cash");
  const canDelete =
    isSuperAdmin || userPermissions.includes("delete_custom_cash");
  const canApprove =
    isSuperAdmin || userPermissions.includes("approve_custom_cash");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const apiUrl =
        process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000/api";
      // 🚀 ดึงเฉพาะประเภทเอกสารนี้เท่านั้น
      const res = await fetch(`${apiUrl}/sale-documents?type=custom_cash`, {
        headers: {
  ...
  const executeRevise = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/sales/custom-cash-sales/${result.data.id}/edit`);
      } else {
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-[11px] mt-0.5">
              จัดการเอกสารบิลเงินสดที่ปรับแต่งโลโก้/ผู้ขายได้ต่อเอกสาร
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {canCreate && (
            <Link href="/sales/custom-cash-sales/create">
              <button className="flex justify-center h-10 px-5 py-2 w-full md:w-auto gap-2 text-sm font-medium items-center text-white bg-teal-600 hover:bg-teal-700 shadow-sm shadow-teal-600/20 rounded-full cursor-pointer transition-all hover:scale-102 transition-transform disabled:opacity-50">
                <Plus className="w-4 h-4" /> สร้างบิลเงินสด
              </button>
            </Link>
          )}
        </div>
        ...
        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div className="relative w-full max-w-sm">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 rounded-xl h-10 border border-slate-200 focus:border-teal-500 focus:ring-2 focus:ring-teal-100 outline-none text-sm transition-colors"
            />
          </div>
        </div>
                ...
                paginatedDocs.map((doc) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 font-bold text-slate-800">
                      {doc.document_number}
                      {doc.status === "Revised" && (
                        <span className="ml-2 text-[10px] bg-slate-100 text-slate-400 px-1.5 py-0.5 rounded font-medium border border-slate-200">
                          Old Ver.
                        </span>
                      )}
                    </td>
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {canEdit && doc.status === "Pending" && (
                          <AppTooltip label="แก้ไข">
                            <Link
                              href={`/sales/custom-cash-sales/${doc.id}/edit`}
                            >
      ...
      <AppConfirmDialog
        ...
        icon={CopyPlus}
        iconColorClass="bg-teal-50 text-teal-600 border-teal-100/50"
      ...
      <AppConfirmDialog
        ...
        description="ระบบจะตัดสต๊อกออกทันทีเมื่ออนุมัติ ยืนยันการอนุมัติบิลเงินสดฉบับนี้ใช่หรือไม่?"
      ...
      <AppConfirmDialog
        ...
        title="ยกเลิกบิลเงินสด?"
        description="ระบบจะคืนสต๊อกให้ถ้าเคยอนุมัติไปแล้ว ยืนยันการยกเลิกบิลเงินสดนี้ใช่หรือไม่?"
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl h-[90vh] shadow-2xl flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800 flex items-center gap-2">
                <FileText className="w-5 h-5 text-teal-500" /> เอกสาร PDF
              </h3>
              <button
                onClick={() => {
