...
export default function CreditNoteListPage() {
  ...
  const canCreate =
    isSuperAdmin || userPermissions.includes("create_credit_note");
  const canEdit = isSuperAdmin || userPermissions.includes("edit_credit_note");
  const canDelete =
    isSuperAdmin || userPermissions.includes("delete_credit_note");
  const canApprove =
    isSuperAdmin || userPermissions.includes("approve_credit_note");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/sale-documents?type=credit_note`, {
        headers: {
  ...
  const executeRevise = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/sales/credit-notes/${result.data.id}/edit`);
      } else {
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-[11px] mt-0.5">
              จัดการเอกสารใบลดหนี้ (รับคืนสินค้า) สำหรับลูกค้า
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {/* 🧾 คืนสินค้าจากใบลดหนี้นี้ — เดิมไม่มีปุ่มเข้าเลย ต้องเข้าทางปุ่มทั่วไปที่หน้าสร้างใบคืนสินค้า
                            แล้วเลือกใบลดหนี้เอง ตอนนี้ prefill ให้ตรงจากตรงนี้เลย (เฉพาะใบที่อนุมัติแล้วเท่านั้น) */}
                        {doc.status === "Approved" && (
                          <AppTooltip label="คืนสินค้าจากใบลดหนี้นี้">
                            <Link
                              href={`/sales/stock-returns/create?credit_note_id=${doc.id}`}
                            >
                              <button className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-colors cursor-pointer">
                        ...
                        {canEdit && doc.status === "Pending" && (
                          <AppTooltip label="แก้ไข">
                            <Link href={`/sales/credit-notes/${doc.id}/edit`}>
      ...
      <AppConfirmDialog
        ...
        description="ระบบจะรับสต๊อกคืนเข้าคลังทันทีเมื่ออนุมัติ ยืนยันการอนุมัติใบลดหนี้ฉบับนี้ใช่หรือไม่?"
      ...
      <AppConfirmDialog
        ...
        description="ระบบจะดึงสต๊อกออกถ้าเคยอนุมัติไปแล้ว ยืนยันการยกเลิกใบลดหนี้นี้ใช่หรือไม่?"
