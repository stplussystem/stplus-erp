...
export default function CreditNoteCreatePage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasPermission = perms.some((p: any) =>
        typeof p === "string"
          ? p === "create_credit_note"
          : p?.name === "create_credit_note",
      );
  ...
  const handlePreviewPDF = async () => {
    ...
    try {
      ...
      const { paperSize, letterLayout } = getPaperSizeConfig(companySettings, "credit_note");
    ...
    <div className="w-full max-w-full px-4 py-4 text-foreground">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 print:hidden gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-md font-bold tracking-tight">สร้างใบลดหนี้</h1>
