...
export default function LoanIssueListPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasPermission = perms.some((p: any) =>
        typeof p === "string" ? p === "view_loan_issue" : p?.name === "view_loan_issue",
      );
  ...
  const canCreate = isSuperAdmin || userPermissions.includes("create_loan_issue");
  ...
  const fetchCompany = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/company`, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      const token = getToken();
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000/api";
      const res = await fetch(`${apiUrl}/sale-documents?type=loan_issue`, {
  ...
  const handlePrint = async (docId: number) => {
    ...
    try {
      const token = getToken();
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000/api";
      const res = await fetch(`${apiUrl}/sale-documents/${docId}`, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
        ...
        const selectedContact = fullDoc.contact || { business_name: fullDoc.borrower_name, mobile: fullDoc.borrower_phone };
  ...
  const filteredDocs = documents.filter(
    (doc) =>
      ...
      doc.borrower_name?.toLowerCase().includes(searchQuery.toLowerCase()),
  );
  ...
  return (
    <div className="w-full max-w-full px-4 py-4 text-foreground">
          ...
          {canCreate && (
            <Link href="/loans/issues/create">
              <button className="flex justify-center h-10 px-5 py-2 w-full md:w-auto gap-2 text-sm font-medium items-center text-white bg-amber-600 hover:bg-amber-700 shadow-sm shadow-amber-600/20 rounded-full cursor-pointer transition-all hover:scale-102 transition-transform disabled:opacity-50">
                <Plus className="w-4 h-4" /> สร้างใบยืมสินค้า
              </button>
            </Link>
          )}
        </div>
          ...
          <table className="w-full text-sm text-left whitespace-nowrap">
            <thead className="text-xs text-muted-foreground uppercase bg-muted/50 border-b border-border">
              <tr>
                ...
                <th className="px-6 py-4 font-bold text-center">ทิศทาง</th>
                <th className="px-6 py-4 font-bold">ผู้ยืม/ลูกค้า</th>
                <th className="px-6 py-4 font-bold">วันที่ต้องคืน</th>
      ...
      <AppConfirmDialog
        ...
        description="ระบบจะจองสินค้าไว้ทันทีเมื่ออนุมัติ (ของยังอยู่ในระบบ แค่ถูกล็อก) ยืนยันการอนุมัติใบยืมสินค้าฉบับนี้ใช่หรือไม่?"
        ...
        onSecondary={() => {
          ...
          setApproveTarget(null);
          if (id) router.push(`/loans/issues/${id}/edit`);
        }}
      />
      ...
      <AppConfirmDialog
        ...
        title="ยกเลิกใบยืมสินค้า?"
        description="ระบบจะปลดล็อกสินค้าให้ถ้าเคยอนุมัติไปแล้ว ยืนยันการยกเลิกใบยืมสินค้านี้ใช่หรือไม่?"
