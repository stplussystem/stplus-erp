    ...
    <div className="w-full px-4 py-4 text-foreground">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6 items-start">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-4 lg:sticky lg:top-4 print:hidden">
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1">ใกล้ครบกำหนดภายใน (วัน)</label>
            <AppSelect
              value={days}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          {loading ? (
            ...
            <div className="text-center py-14">
              ...
              <p className="text-slate-400">ไม่มีสินทรัพย์ที่ใกล้ครบกำหนดบำรุงรักษาตามเงื่อนไขที่เลือก</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {rows.map((row) => (
                    <tr key={row.asset.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="px-6 py-4 font-bold text-slate-800">{row.asset.name}</td>
                      <td className="px-6 py-4 text-slate-600">{row.asset.category || "-"}</td>
                      <td className="px-6 py-4 text-slate-600">
                        {row.asset.responsible_user?.name ? (
                          <span className="flex items-center gap-1.5">
                            <UserIcon className="w-3.5 h-3.5 text-slate-400" /> {row.asset.responsible_user.name}
                          </span>
                        ) : (
                          "-"
                        )}
