...
import { Truck, ArrowLeft, Package, RefreshCw, FileSpreadsheet } from "lucide-react";
...
function TopSuppliersReportPageContent() {
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");
  const [rows, setRows] = useState<TopSupplierRow[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/top-suppliers?${buildParams()}`, {
  ...
  const handleExport = async () => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/top-suppliers/export?${buildParams()}`, {
      ...
      a.href = url;
      a.download = `top_suppliers_${Date.now()}.xlsx`;
    ...
    <div className="w-full px-4 py-4 text-foreground">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6 print:hidden">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-md font-bold tracking-tight">รายงานซัพพลายเออร์อันดับต้น</h1>
            <p className="text-muted-foreground text-[11px] mt-0.5">
              ซัพพลายเออร์ที่มียอดสั่งซื้อรวมสูงสุด (ใบสั่งซื้อที่ยืนยัน/รับของครบแล้ว)
            </p>
          </div>
        </div>
        ...
        <div className="bg-card rounded-2xl shadow-sm border border-border p-5">
          {loading ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-muted-foreground">ยังไม่มีข้อมูลการจัดซื้อ</p>
            </div>
          ) : (
...
export default function TopSuppliersReportPage() {
  return (
    <RoleRouteGuard permission="view_reports_top_suppliers">
      <TopSuppliersReportPageContent />
    </RoleRouteGuard>
  );
}
