...
function ApAgingReportPageContent() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/ap-aging?${buildParams()}`, {
  ...
  const handleExport = async () => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/ap-aging/export?${buildParams()}`, {
      ...
      a.href = url;
      a.download = `ap_aging_${Date.now()}.xlsx`;
