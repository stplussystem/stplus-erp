...
import { Users, Package, RefreshCw, FileSpreadsheet, Printer } from "lucide-react";
...
function TopCustomersReportPageContent() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/top-customers?${buildParams()}`, {
  ...
  const handleExport = async () => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/top-customers/export?${buildParams()}`, {
      ...
      a.href = url;
      a.download = `top_customers_${Date.now()}.xlsx`;
    ...
    <div className="w-full px-4 py-4 text-foreground">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6 print:hidden">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-md font-bold tracking-tight">รายงานลูกค้าซื้อสูงสุด</h1>
            <p className="text-muted-foreground text-[11px] mt-0.5">
              ลูกค้าที่มียอดซื้อรวมสูงสุด (ใบกำกับภาษี/บิลเงินสด/ใบเสร็จรับเงิน) เรียงจากมากไปน้อย
            </p>
          </div>
        </div>
...
export default function TopCustomersReportPage() {
  return (
    <RoleRouteGuard permission="view_reports_top_customers">
