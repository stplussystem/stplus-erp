...
export default function RentalJobEditPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [jobRes, usersRes, jobTypesRes] = await Promise.all([
          fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/rental-jobs/${rentalJobId}`,
            { headers },
          ),
        ...
        console.error("Error fetching rental job:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/rental-jobs/${rentalJobId}`);
      } else {
    ...
    <div className="w-full px-4 py-4 text-foreground">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-md font-bold tracking-tight">แก้ไขงานเช่า</h1>
          </div>
        </div>
        <button
          onClick={() => router.push(`/rental-jobs/${rentalJobId}`)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div>
            ...
            <AppSelect
              ...
              onValueChange={(v) =>
                setFormData({ ...formData, stage: v === "__none__" ? "" : v })
              }
              options={[
